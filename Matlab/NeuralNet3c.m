...
for ep = ep_start:Nb
    ...
    for im = im_train
        ...
        if sum(X) ~= 0
            predAcc_train(ep) = predAcc_train(ep) + 100*(sum((Yh > threshold) == Y & X == 1)/sum(X))/batchSize;
        end
    ...
    for k = im_test
        ...
        if sum(X) ~= 0
            predAcc_test(ep) = predAcc_test(ep) + 100*(sum((Yh > threshold) == Y & X == 1)/sum(X))/batchSize;
        end
        jaccard = 1 - pdist([((Yh > threshold).*X)'; Y'], 'jaccard');
        if ~isnan(jaccard)
